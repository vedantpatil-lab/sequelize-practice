...
const followUser = async (req, res) => {
    try {
        ...
        if (id === req.userId) return res.status(400).json({ message: "You cannot follow Yourself !" })
        ...
        console.error("Error follow user")
        res.status(500).json({ message: "Server Error !" })
    }
}
...
const unfollowUser = async (req, res) => {
    try {
        ...
        if (!exists) return res.status(400).json({ message: "You are not following this user" })
        await Follow.destroy({
            where: { followerId: req.userId, followingId: id }
        })
        res.status(200).json({ message: "You've unfollowed this user !" })
    } catch (error) {
        console.error("Error Unfollow User")
        res.status(500).json({ message: "Server Error !" })
    }
}
...
module.exports = { register, login, logout, refreshToken, profile, followUser, unfollowUser, deleteUser, getUsers}
